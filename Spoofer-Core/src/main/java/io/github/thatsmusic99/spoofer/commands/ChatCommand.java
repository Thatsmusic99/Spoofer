...
public class ChatCommand implements ISubCommand {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label,
        ...
        if (args.length < 3) {
            Spoofer.sendWarnMessage(sender, "You need to specify the chat message for the player.");
            return false;
        }

        String message = Utilities.getStringFromArray(args, 2);
        player.getBukkitPlayer().chat(message);
        Spoofer.sendMessage(sender, "Player " + args[1] + " has sent the message \"" + message + "\".");
        return true;
    }
